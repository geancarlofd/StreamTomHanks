...
window.onload = function(){
    ...
    $(".check").click(function () {
        console.log("limpou");
        document.getElementById('filmesFiltro').innerHTML = "";

        var referencia = $(this).attr("id");
        if (document.getElementById(referencia).checked) {
            var verificador = false;

            for (var i = 0; i < filtroArray.length; i++) {
                if (filtroArray[i] == referencia) {
                    verificador = true;
                }
            }
            if (verificador == false) {
                filtroArray.push(referencia);
            }
            ...
            filtroArray.splice(cont, 1);
            console.log(filtroArray);
        }
        console.log("TamanhoFILTRO: " + filtroArray.length)
...
function cards(retorno){
    console.log("ENTROU"+retorno.tamanho)
    var cards = '';

    for (var i = 0; i < retorno.tamanho; i++) {

        cards += '<div class="imagens imagensFilmes" id_filmes="' + retorno[i].id + '">';
...
function filmes() {
    ...
    $.ajax({
        type: "POST",
        dataType: "json",
        url: "../../php/filtro/filtroAll.php",
        success: function (retorno) {
            if (retorno.funcao == "AllFilmes") {
                if (retorno.status == "s") {
                    cards(retorno);
                }
            }
...
function filme(){
    ...
    for(var i = 0; i < filtroArray.length; i++){
        elemento = filtroArray[i];

        $.ajax({
            type: "POST",
            dataType: "json",
            url: "../../php/filtro/filtro.php",
            data: { elemento: elemento },
            success: function (retorno) {
                if (retorno.funcao == "filtro"){
                    if(retorno.status == "s"){
                        cards(retorno);
                    }
                }
